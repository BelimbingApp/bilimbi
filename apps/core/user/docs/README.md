...
| `list_unaffiliated_users(scope, actor, opts)` | Operator-only list of users without company affiliation (`admin.user.unaffiliated.manage`) |
| `get_unaffiliated_user(scope, actor, user_id)` | Operator-only read of one unaffiliated user |
| `create_unaffiliated_user(scope, actor, attributes)` | Operator-only creation of an unaffiliated user account |
| `assign_unaffiliated_user(scope, actor, target_company_id, user_id, opts)` | Assign an unaffiliated user to a live company and optional employee |
| `reassign_user_company(scope, actor, current_company_id, target_company_id, user_id, opts)` | Reassign a user to a target live company with ascending lock ordering |
| `clear_user_company(scope, actor, current_company_id, user_id, opts)` | Move an affiliated user back to unaffiliated state |
| `admin_change_password(scope, actor, company_id, user_id, new_password, opts)` | Admin password reset with token rotation and session invalidation |
