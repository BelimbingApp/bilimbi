defmodule BilimbiWeb.EmployeeShowTest do
  ...
  test "deletes an ordinary employee", %{conn: conn, employee: employee} do
    ...
    {:ok, _index, html} =
      view
      |> element("#employee-delete")
      |> render_click()
      |> follow_redirect(conn, ~p"/employees")

    assert html =~ "John Doe was deleted."
  end
  ...
  test "refuses to delete the platform orchestrator", %{conn: conn} do
    ...
    html = view |> element("#employee-delete") |> render_click()

    assert html =~ "The platform orchestrator cannot be deleted."
    {:ok, scope} = Tenancy.scope(41)
