defmodule BilimbiWeb.EmployeeFormTest do
  ...
  test "creates an employee through the domain API", %{conn: conn} do
    grant_capabilities!("admin.employee.create")
    ...
    assert html =~ "Grace Hopper was created."
  ...
  test "edits an employee", %{conn: conn} do
    ...
    grant_capabilities!("admin.employee.update")
    ...
    {:ok, _show, html} =
      ...
      |> render_submit()
      |> follow_redirect(conn, ~p"/employees/#{employee.id}")

    assert html =~ "John Richard Doe was updated."
  end
end
