defmodule BilimbiWeb.AppShellJsTest do
  ...
  test "pins hydrate from the authenticated API and migrate legacy browser pins", %{
    source: source
  } do
    assert source =~ ~S[this.pinRequest("/api/pins")]
    assert source =~ "readLegacyPinnedItems"
    assert source =~ ~S[window.localStorage.removeItem?.(PINNED_STORAGE)]
    refute source =~ "savePinnedItems()"
  end

  test "pinned pages can be reordered with keyboard controls and announcements", %{source: source} do
    assert source =~ "onPinnedKeyboard"
    assert source =~ "data-nav-move"
    assert source =~ "Moved ${label} ${direction}."
    assert source =~ "focusPinnedKey"
    assert source =~ ~S["/api/pins/reorder"]
  end

  test "impersonated shells make pin writes inert and route reads discard stale pins", %{
    source: source
  } do
    assert source =~ ~S[this.impersonating = this.el.dataset.impersonating === "true"]
    assert source =~ "this.isServedUrl(pin.url)"
    assert source =~ "pin.disabled = this.impersonating"
    assert source =~ "if (this.impersonating) return"
  end
