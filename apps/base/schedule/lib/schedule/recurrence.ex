defmodule Bilimbi.Base.Schedule.Recurrence do
  ...
  def previous_occurrence(%Definition{} = definition, %DateTime{} = now) do
    with {:ok, local_now} <- DateTime.shift_zone(now, definition.timezone, @database) do
      find_previous(definition, DateTime.to_naive(local_now), now, 0)
    else
  ...
  defp find_previous(definition, local_now, now, attempts) do
    with {:ok, candidate} <- Scheduler.get_previous_run_date(definition.cron, local_now) do
