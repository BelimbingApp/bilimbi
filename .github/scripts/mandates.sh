...
mapfile -t scan_roots < <(find apps -type d -name lib 2>/dev/null | sort)

if [ "${#scan_roots[@]}" -eq 0 ]; then
  echo "FAIL mandates: no lib directories found under apps/ — run from the repository root, or the layout has changed"
  exit 1
fi
...
tenant_hits=$(grep -rnE 'tenant_id\s*==\s*\^' "${scan_roots[@]}" 2>/dev/null |
...
atom_hits=$(grep -rn 'String\.to_atom(' "${scan_roots[@]}" 2>/dev/null || true)
