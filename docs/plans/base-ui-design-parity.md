...
`apps/web/test/bilimbi_web/multi_select_dismiss_js_test.exs`. The shipped specimen is
held by `apps/base/ui/web_test/design_library_live_test.exs`, which asserts the reveal
control on `/system/design-library/components#component-inputs`, the sibling secret
field that has none, and the revealable field carrying no server-held value.
