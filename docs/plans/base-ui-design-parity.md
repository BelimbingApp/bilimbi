...
Implementation and adoption: `Bilimbi.Base.UI.ShellComponents` supplies the account disclosure, access warning and display controls to `Layouts.app`. The authenticated Web hook resolves fresh session identity before preference writes and uses the existing User and DateTime APIs; no schema or preference-key migration was needed. `Bilimbi.Core.User.DisplayPreferences` owns the account's theme and time display: one resolved snapshot and one durable write, used by the shell hook, the `/api/theme` adapter and the appearance screen alike. The impersonation refusal is stated once there, so an impersonated session writes no display preference from any of the three, and the shell replaces its controls with the active time display and says why. The authenticated scope carries that one snapshot, which stamps the root layout, fills the timestamp display context and renders both the top-bar controls and the appearance form's theme and time-display fields; every confirmed write refreshes it, so no surface keeps a second copy. Timestamps given `display` from a tracked assign follow a saved mode at once; the rest follow as the page next renders them. Every authenticated route adopts the shell, with direct browser checks on `/dashboard`, `/companies`, `/settings/password` and all four Design Library routes. `/system/design-library/components#component-shell` demonstrates a live timestamp and points at the page's own top-bar display controls and bottom-left account circle rather than duplicating either. `/system/design-library/design-spec#spec-shell` records the accepted user behavior.
...
- Actual timezone saves changed the dashboard's rendered timestamps between UTC and Asia/Kuala_Lumpur company time. Browser evidence in this list was gathered at `06fe012`, before the save switched from navigation to an in-place patch; re-verification of the save interaction belongs to the final tree. A deliberately invalid browser-side choice was rejected by the server, kept the previous theme selected and exposed “Could not save display preference…”; retry then succeeded. System theme followed light/dark media changes without a preference write. Fresh sign-in restored the saved theme and timezone mode.
