...
| NAV-05 [(contradicted)](#targets-the-evidence-contradicts) | User account and scope menu | The bottom-left user circle opens the account disclosure with signed-in name and identifier, current company and tenant, change password and sign out; no scope switcher because the current identity holds exactly one scope (multi-company membership policy deferred) | On user-circle activation, show signed-in name and identifier, current company and tenant, change password and sign out. Show scope switching only when more than one permitted scope exists. |
...
| CMP-01 [(contradicted)](#targets-the-evidence-contradicts) | Operational index page | Complete operational indexes already ship (`/companies`, `/users`, `/audit/mutations`, `/notifications`); the shared composition is not extracted, and `/audit/mutations` hand-writes a Previous/Next pager instead of the shared pagination component (Lane A) | Extract the shared index composition (header, filters, table, actions, empty/loading/error, pagination) from the shipping indexes. |
