...
| Top-bar timezone | Adopt adapted | Belimbing's Company/Local/Stored choices and clock meaning are useful. Bilimbi uses the existing Base DateTime preference contract and re-renders the page from the server after a confirmed save, so every timestamp follows the new mode without client rewriting. Explicit per-element display and calendar dates remain independent. |
...
Implementation and adoption: `Bilimbi.Base.UI.ShellComponents` supplies the account disclosure, access warning and display controls to `Layouts.app`. The authenticated Web hook resolves fresh session identity before preference writes and uses the existing User and DateTime APIs; no schema or preference-key migration was needed. An impersonated session writes no display preference: the shell replaces the controls with the active time display and says why, and `ShellPreferences.save/3` refuses the write for the shell hook and the `/api/theme` adapter alike. The authenticated scope carries one resolved theme-and-display snapshot, which stamps the root layout, fills the timestamp display context and renders both the top-bar controls and the appearance form, so no surface keeps a second copy. Every authenticated route adopts the shell, with direct browser checks on `/dashboard`, `/companies`, `/settings/password` and all four Design Library routes. `/system/design-library/components#component-shell` renders the real controls and a live timestamp, and points at the live bottom-left account circle rather than duplicating it. `/system/design-library/design-spec#spec-shell` records the accepted user behavior.

Validation evidence:

- `mix precommit` passed on 2026-09-14: all Base/Core suites, 547 Web integration tests and six installed contribution snapshots. `mix format --check-formatted` and `git diff --check` passed. Meaningful coverage includes revoked-session rejection, per-user isolation, remount persistence, invalid input and failed HTTP writes, ordinary/operator/impersonated warnings, duplicate-work prevention, disconnect/unknown-result feedback, and the server re-render that follows a saved time display.
...
- Actual timezone saves changed existing dashboard timestamps immediately between UTC and Asia/Kuala_Lumpur company time, through the server re-render the save triggers. The library timestamp also followed its own real display control. A deliberately invalid browser-side choice was rejected by the server, kept the previous theme selected and exposed “Could not save display preference…”; retry then succeeded. System theme followed light/dark media changes without a preference write. Fresh sign-in restored the saved theme and timezone mode.
...
Checked visual evidence: [expanded production shell, light](evidence/base-ui-shell/companies-expanded-light.png), [narrow account drawer, dark](evidence/base-ui-shell/narrow-account-dark-final.png). Screenshots supplement the exercised controls and tests; they are not the interaction evidence by themselves. The two Design Library screenshots taken during this slice were removed with the duplicated account specimen they showed; the library section now points at the same production shell the remaining screenshots capture.
