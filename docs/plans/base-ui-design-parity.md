...
- **OVR-01** — "Missing" understates, and moving toward Belimbing would be a
  ...
  verified. The shared `<.modal>` has since shipped, so the row above carries that
  shipped state instead of the accepted "Missing" wording.
