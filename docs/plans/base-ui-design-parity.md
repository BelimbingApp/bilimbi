...
- [x] Lane A — audit FND, LAY and NAV against both applications and representative shell/page routes. `{crewmate}` — FND-06 was excluded from this lane by its brief, which told the scout not to audit it or build an icon inventory; the icon vocabulary was inventoried separately by a crewmate scout (`parity-icon-inventory/report.md`, 48 actions, held outside the repository like the lane reports), implemented through the registry by `{codex-terra-icons-1/gpt-5.6-terra}` (#715) and held to the Heroicons the build emits by `{fm/icon-registry-name-validity/opus-5}` (#727).
- [x] Lane B — audit ACT, FBK and OVR, including loading, dismissal, confirmation and recovery. `{crewmate}`
- [x] Lane C — audit INP and INT, including field shell, keyboard behavior, editing and rich choices. `{crewmate}`
- [x] Lane D — audit DAT, CMP and GFX, including actual index, form, detail, authentication and dashboard flows. `{crewmate}`
