...
| ACT-03 | Destructive entry and acknowledgement | Consequence-first `<.confirm_dialog>` on the reference-type deletes and the address unlinks; the other destructive controls still confirm natively and convert one owner at a time; typed acknowledgement stays unbuilt until a high-risk workflow needs it | Standardize consequence copy, confirmation and typed acknowledgement where risk requires it. |
...
| OVR-02 [(contradicted)](#targets-the-evidence-contradicts) | Confirmation modal | `<.confirm_dialog>` on the shared modal: consequence-first `alertdialog`, calm danger confirm, Cancel first and focused, Escape cancels, no typed acknowledgement; adopted by the reference-type deletes and the address unlinks, with the other native confirms pending conversion | Add consequence-first confirmation without copying Belimbing's accessibility gaps. |
