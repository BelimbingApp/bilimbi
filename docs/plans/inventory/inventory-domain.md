...
The Manufacturing Domain is separate and has its own plan. It supplies optional execution, order or batch, and Work Centre/Resource references when it posts. Stock stores those references as opaque context; it does not resolve or interpret them. See [`manufacturing-domain.md`](../manufacturing/manufacturing-domain.md) for the Production module and its ownership.
...
- **Option B — store genealogy in Manufacturing:** production trace is convenient, but Stock requires Manufacturing and warehouse transformations lose the same lineage.
