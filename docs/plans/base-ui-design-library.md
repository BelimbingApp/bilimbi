...
- [x] Drop the parity ledger's row identifiers from every operator-facing heading and description in the Components and Design Spec areas, including the eleven family headings introduced by #724 — the eleven-family grouping itself stays — and give the canonical table specimen the sortable single-card shape `/users` uses. `{fm/designlib-catalog-ids-in-ui/opus-5}`
