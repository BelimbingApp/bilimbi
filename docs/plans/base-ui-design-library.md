...
The canonical table entry shows the shape `/users` uses rather than a weaker one: the table sits unframed inside one card with its pagination, and every data column heading sorts, with a repeated click flipping direction, a newest-first default on the timestamp column and a sort that returns to page one. Its example rows carry distinct timestamps so that sort is observable. Because both table specimens now pass `framed={false}`, no specimen shows the framed default that screens such as Audit mutations and the Schedule task board still render; a second specimen for it is open work.
...
- [x] Drop the parity ledger's row identifiers from the Application shell headings on Components and Design Spec, and give the canonical table specimen the sortable single-card shape `/users` uses. `{fm/designlib-catalog-ids-in-ui/opus-5}`
